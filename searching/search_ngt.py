...
@click.command()
@click.option(
    "-d", "--descriptor_path", help="path to descriptor", type=click.File("rb")
)
@click.option(
    "-k",
    "--knn_path",
    help="path where the knn model will be stored",
    type=str,  # click.File("wb"),
)
@click.option(
    "--save/--no-save", help="should save or not the knn model", default=False
)
@click.pass_context
def train(ctx, descriptor_path, knn_path, save):
    descriptor = pickle.load(descriptor_path)
    dim = 128  # descriptor["features"].shape[1]
    enc_path = str(knn_path).encode("utf_8")
    
    logger.info(f"create NGT index in {enc_path}")
    ngtpy.create(enc_path, dim)
    index = ngtpy.Index(enc_path)
    index.batch_insert(descriptor["features"])
...
@click.option(
    "-k", "--knn_path", help="path to knn model", type=str
)  # click.File('rb')
...
def nearest(ctx, k_nearest, source, knn_path, vgg16_path, descriptor_path):
    ...
    indices, distances = zip(*index.search(reduced_features, k_nearest))
    print(indices, distances)
    # response = list(op.itemgetter(*indices[0])(descriptor['filepaths']))
    ids = np.array(descriptor["filepaths"])
    # print(descriptor["filepaths"])
    # print(descriptor.keys())
    response = ids[list(indices)].tolist()
    print("\n".join(response))
...
@click.option(
    "--score", help="threshold value for weights metrics", default=0.5, type=float
)
@click.option(
    "--threshold",
    help="threshold value for SIFT keypoint match",
    default=0.75,
    type=float,
)
